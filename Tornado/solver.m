function [results]=solver(state,geo,lattice)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~isvalid(h), results = []; return, end %closed early
...
[w3 DW]=fastdw(lattice,2,h);	                %Calculating downwash on vorticies
if isempty(DW), results = []; return, end       %closed early
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5five
function[dw,DW,h]=fastdw(lattice,n,h)
...
if n==1, h=waitbar(0,'Running 3-D Vortex Lattice Method...','Name','Tornado'); end
for j=1:(vsize-1)

    %Wait Bar
    if ~isvalid(h), dw = []; DW = []; return, end %closed early
    waitbar(((vsize-1)*(n-1)+j)/((vsize-1)*2))
