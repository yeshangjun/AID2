...
if isempty(findall(0,'Type','Figure')) %如果没有uifigure
    ...
    res=get(0,'ScreenSize'); unit = 'm'; 
    % w = res(3); h = res(4); %用屏幕分辨率有不同结果
    w=960; h=600; % 固定分辨率
    % 生成一个空白的窗口，定义名称，位置等
    f=figure('Name','飞机目标快速气动评估软件 v1.0','NumberTitle','off',...
        'MenuBar','none','Position',[(res(3)-w)/2,(res(4)-h)/2,w,h],'Resize','off',...
...
% str = '手动
%     换行'];
c=uicontrol(Body,'Style','pushbutton','String','手动调整', 'Units',...
...
c=uicontrol(Body,'Style','text','String','部件序号','Units','normalized',...
    'Position',[x0,y0-3.6*dy,wbedit,h*1.5]);
% [wrappedtext,position] = textwrap(c,{c.String},2);
c.String = {'部件';'序号'};

c=uicontrol(Body,'Style','text','String',['位置, ',unit],'Units',...
    'normalized','Position',[x0+dx-0.02,y0-3.6*dy,wbedit+0.04,h*1.5]);
c.String = {'位置, ';unit};
c=uicontrol(Body,'Style','pushbutton','String','截面形状','Units',...
    'normalized','Position',[x0+2*dx,y0-3.6*dy,wbedit*1.5,h*1.5],...
    'Callback',{@setting,'shape'});
% c.String = {'截面';'形状'};
y0 = y0-4*dy;
